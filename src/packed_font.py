...
def _load_packed_font(font_name):
    font = None
    with open(f'{_fontDir}{font_name}.pf', 'rb') as f:
        header = f.read(4)
...
def read_bit(data, bit_position):
    """
    Read a specific bit from binary data

    Parameters:
    data (bytes): Binary data
    bit_position (int): Position of the bit to read (0-indexed)

    Returns:
    int: Value of the bit (0 or 1)
    """
    byte_index = bit_position // 8  # Determine which byte the bit is in
    bit_index = 7 - (bit_position % 8)  # Bit position within the byte (MSB is 0)

    # Check if the bit position is valid
    if byte_index >= len(data):
        raise IndexError("Bit position out of range")

    # Extract the byte and then the specific bit
...
def text(display, text, x, y, max_width=0, horiz_align=0, max_height=0, vert_align=0, c=1):
    ...
    for char in text:
        ...
        for i in range(height * width):
            bitValue = read_bit(data, start_index * 8 + i)
            lx = i % width
            ly = math.floor(i//width)
            if bitValue & 1:
                display.pixel(x + lx, y + ly, c)
        x += width
